(() => {
  ...
  function applyInboundResult({ session_id, task_id, lap_id, task_state, from_app, event_id = null }) {
    ...
    const specialistSource = from_app || 'specialist';
    const receivedAt = iso();
    c.active_app = 'ready-set';
    c.active_task_id = task.task_id;
    if (lap_id) c.active_lap_id = lap_id;

    task.last_specialist_result = {
      source_app: specialistSource,
      event_id: event_id || null,
      lap_id: lap_id || null,
      raw_task_state: task_state || null,
      normalized_task_state: normalized,
      received_at: receivedAt
    };

    if (normalized) setTaskState(task.task_id, normalized, specialistSource);
    if (['COMPLETED','WAITING_FOR_PARENT','BLOCKED'].includes(normalized)) {
      endActiveLap('SPECIALIST_RESULT', normalized);
    }
    if (event_id) c.applied_event_ids = [...(c.applied_event_ids || []), event_id].slice(-200);
    emit('APP_RETURN', {
      from: specialistSource,
      event_id: event_id || null,
      raw_task_state: task_state || null,
