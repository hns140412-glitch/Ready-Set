  ...
  function createPlanner(storage){
    ...
    function upsertDatedTodo(input={}){
      ...
      return mutate(s=>{
        ...
        const item={
          ...
          source_range:cleanText(input.source_range)||null,
          workbook_ref_id:cleanText(input.workbook_ref_id)||null,
          ...
          activity_sequence:Array.isArray(input.activity_sequence)?[...input.activity_sequence]:[],
          matched_domain:cleanText(input.matched_domain)||null,
          method_variant:cleanText(input.method_variant)||null,
          concept_skill_target:cleanText(input.concept_skill_target)||null,
          divisible_boundary:cleanText(input.divisible_boundary)||null,
          cognitive_load_profile:Array.isArray(input.cognitive_load_profile)?[...input.cognitive_load_profile]:[],
          confidence:Number.isFinite(input.confidence)?input.confidence:null,
          unresolved_flags:Array.isArray(input.unresolved_flags)?[...input.unresolved_flags]:[],
          review_lexical_ids:Array.isArray(input.review_lexical_ids)?[...new Set(input.review_lexical_ids.map(cleanText).filter(Boolean))].slice(0,24):[],
    ...
    function linkTodayItems(todoIds=[],options={}){
      ...
      return load().dated_todos.filter(x=>ids.has(x.todo_id)&&x.date===date&&allowedStates.has(x.state)).map(x=>({
        ...
        review_lexical_ids:Array.isArray(x.review_lexical_ids)?[...x.review_lexical_ids]:[],
      ...
      return mutate(s=>{
        ...
        for(const unit of units){
          ...
          proposals.push({
            ...
            review_lexical_ids:Array.isArray(unit.review_lexical_ids)?[...unit.review_lexical_ids]:[],
      ...
      return mutate(s=>{const run=s.allocation_runs.find(x=>x.allocation_run_id===runId&&x.version==='PLANNER_V2');if(!run)return {ok:false,reason:'ALLOCATION_RUN_NOT_FOUND'};
        ...
        for(const p of run.proposals.filter(x=>x.decision==='PROPOSE')){
          ...
          if(!todo){todo={
            ...
            review_policy:p.review_policy,review_lexical_ids:Array.isArray(p.review_lexical_ids)?[...p.review_lexical_ids]:[],parent_help_dependency:p.parent_help_dependency,
