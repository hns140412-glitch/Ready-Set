(function(root,factory){
  ...
  function createDomain(storage){
    ...
    function supersedeCurrentInterpretation(s,fact,reason='FACT_UPDATED'){
      const analysisId=clean(fact.current_analysis_id);
      if(!analysisId)return null;
      const analysis=s.analyses[analysisId];
      if(analysis){
        analysis.state='SUPERSEDED';
        analysis.superseded_at=now();
        analysis.supersede_reason=reason;
        for(const unitId of analysis.learning_unit_ids||[]){
          const unit=s.learningUnits[unitId];
          if(unit&&unit.state!=='SUPERSEDED'){
            unit.state='SUPERSEDED';
            unit.superseded_at=now();
            unit.supersede_reason=reason;
          }
        }
      }
      fact.previous_analysis_ids=Array.isArray(fact.previous_analysis_ids)?fact.previous_analysis_ids:[];
      if(!fact.previous_analysis_ids.includes(analysisId))fact.previous_analysis_ids.push(analysisId);
      fact.current_analysis_id=null;
      fact.analysis_state='REVISION_PENDING_CONFIRMATION';
      fact.fact_revision=Math.max(1,Number(fact.fact_revision)||1)+1;
      fact.planner_revision_pending=true;
      fact.revision_started_at=now();
      fact.revision_reason=reason;
      return analysisId;
    }
    ...
    function upsertTalentPackage(input={}){
      ...
      return mutate(s=>{const packageId=clean(input.package_id)||id('talent_pkg');const factIds=[];
        for(const subject of TALENT_BOOKS){const b=byName.get(subject),assignmentId=clean(b.assignment_id)||id('assignment');
          ...
          if(existing)supersedeCurrentInterpretation(s,f,'PARENT_FACT_UPDATED');
    ...
    function upsertEnglishAssignment(input={}){
      ...
      return mutate(s=>{if(!s.workbookRefs[workbookRefId])throw new Error('workbook ref not found');
        ...
        addClaim(s,f,input.actor||'UNKNOWN',{source_range:clean(input.source_range),weekday_prints:clone(input.weekday_prints||{}),recurring_days:Array.isArray(input.recurring_days)?[...new Set(input.recurring_days.map(Number).filter(x=>x>=0&&x<=6))]:[],components:clone(input.components||{}),teacher_instruction:clean(input.teacher_instruction),...learningContext(input),artifact_refs:artifactRefs,answer_reference_ids:answerRefs},input.provenance||{kind:(input.actor==='CHILD'?'CHILD_INPUT':'PARENT_INPUT')});
    ...
    function markRevisionPropagationComplete(assignmentId,input={}){
      return mutate(s=>{
        const f=s.assignmentFacts[assignmentId];if(!f)throw new Error('fact not found');
        f.planner_revision_pending=false;
        f.revision_propagated_at=now();
        f.revision_propagation={
          fact_revision:Number(f.fact_revision)||1,
          analysis_id:clean(input.analysis_id)||f.current_analysis_id||null,
          allocation_run_id:clean(input.allocation_run_id)||null,
          at:f.revision_propagated_at
        };
        f.updated_at=now();
        return clone(f);
      });
