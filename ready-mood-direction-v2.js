...
function art(t){const pose=t.key==='FEEL_BRIGHT'?'M37 65l-14 18m14-18 17 16':t.key==='FEEL_WARM'?'M36 66l-10 17m10-17 11 18':t.key==='FEEL_CALM'?'M36 66v19m0-12 16 5':t.key==='LOOK_ACTIVE'?'M35 64L19 82m16-18 20 12':t.key==='LOOK_NATURAL'?'M36 65l-8 19m8-19 10 19':'M36 64L22 83m14-19 18 18';const prop=t.key==='FEEL_CALM'?'<path d="M51 72h15v12H51z"/>':t.key==='LOOK_BOLD'?'<path d="M53 70l13 4-9 12z"/>':t.key==='LOOK_ACTIVE'?'<path d="M18 58h10v14H18z"/>':'';return `<svg viewBox="0 0 76 96" aria-hidden="true"><path class="land" d="M2 76c13-15 25-12 36-21 12 10 22 7 36 21v20H2z"/><circle class="sun" cx="61" cy="17" r="8"/><path class="trail" d="M13 96c9-17 25-21 48-27"/><circle class="head" cx="36" cy="36" r="13"/><path class="hair" d="M24 36c0-17 25-20 26 0-5-7-18-9-26 0z"/><path class="body" d="M26 51c7-5 15-5 21 0l7 25H20z"/><path class="limb" d="${pose}"/>${prop}<circle class="spark" cx="13" cy="24" r="2"/><circle class="spark s2" cx="65" cy="45" r="1.5"/></svg>`}
function card(t){return `<button class="takyChoiceCard" data-taky-choice="${t.key}"><span class="takyArt" data-art="${t.key}">${art(t)}</span><strong>${esc(t.label)}</strong><small>${t.keywords.map(esc).join(' · ')}</small></button>`}
function markup(){
...
function wire(m){m.querySelectorAll('[data-taky-choice]').forEach(b=>b.onclick=()=>choose(b.dataset.takyChoice));m.querySelector('[data-taky-reset]')?.addEventListener('click',reset);m.querySelector('[data-taky-generate]')?.addEventListener('click',()=>globalThis.ReadyCharacterCandidateV1?.generate?.())}
function paint(){const i=read();if(i.onboardingStep!=='CHARACTER')return;const m=document.querySelector('#readyCharacterCandidateMount')||document.querySelector('#readyFirstRun .pendingCard');if(!m||!m.querySelector('.ccConsult'))return;if(m.dataset.takyConsult==='v4'&&m.querySelector('.takyConsult'))return;m.dataset.takyConsult='v4';m.innerHTML=markup();wire(m)}
function paintSoon(){queueMicrotask(()=>{const m=document.querySelector('#readyCharacterCandidateMount')||document.querySelector('#readyFirstRun .pendingCard');if(m){m.dataset.takyConsult='';m.dataset.candidateUi='';globalThis.ReadyCharacterCandidateV1?.render?.(m);setTimeout(()=>{m.dataset.takyConsult='';paint()},0)}})}
const style=document.createElement('style');style.textContent=`.takyConsult{display:grid;gap:16px}.takyStep{font:900 11px/1 system-ui;letter-spacing:.14em;color:#3277a7}.takyConsult h2{margin:0;font:950 25px/1.12 system-ui;letter-spacing:-.04em;color:#153d5d}.takyConsult p,.takyConsult small{margin:0;color:#60788b;line-height:1.5}.takyChoiceGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.takyChoiceCard{appearance:none;border:1px solid #dcebf4;border-radius:20px;background:#fff;padding:8px;text-align:left;box-shadow:0 10px 26px #164e7212;overflow:hidden}.takyArt{display:block;aspect-ratio:.86;border-radius:15px;background:linear-gradient(145deg,#eaf8ff,#cfefff 48%,#fff0d7);position:relative;overflow:hidden}.takyArt svg{width:100%;height:100%;display:block}.takyArt .land{fill:#d7ebdf}.takyArt .sun{fill:#ffd66b}.takyArt .trail{fill:none;stroke:#fff;stroke-width:4;stroke-linecap:round}.takyArt .head{fill:#f1c4a3;stroke:#805d49;stroke-width:1.8}.takyArt .hair{fill:#3e4653}.takyArt .body{fill:#4f9ed1;stroke:#256c9b;stroke-width:1.4}.takyArt .limb{fill:none;stroke:#805d49;stroke-width:5;stroke-linecap:round}.takyArt .spark{fill:#fff;stroke:#5aa9cf;stroke-width:1}.takyArt path:not(.land):not(.trail):not(.hair):not(.body):not(.limb){fill:#f3b94f;stroke:#7d623e;stroke-width:1}.takyArt[data-art=FEEL_BRIGHT]{background:linear-gradient(145deg,#d9f6ff,#9bdcf2 55%,#fff0a8)}.takyArt[data-art=FEEL_WARM]{background:linear-gradient(145deg,#fff3db,#f6d8bf 55%,#dff4e9)}.takyArt[data-art=FEEL_CALM]{background:linear-gradient(145deg,#e5f0ff,#c8ddf4 55%,#e8f2d9)}.takyArt[data-art=LOOK_ACTIVE]{background:linear-gradient(145deg,#ddf8ef,#a7e1cf 55%,#ffe39b)}.takyArt[data-art=LOOK_NATURAL]{background:linear-gradient(145deg,#edf4df,#d6e5c3 55%,#f5dfc5)}.takyArt[data-art=LOOK_BOLD]{background:linear-gradient(145deg,#d9ecff,#9fc9ec 55%,#ffd18c)}.takyChoiceCard strong{display:block;margin:10px 2px 4px;font:900 13px/1.25 system-ui;color:#163f60}.takyChoiceCard small{font-size:10px}.takySummary{display:grid;gap:8px}.takySummary div{display:flex;justify-content:space-between;gap:12px;padding:12px 14px;border-radius:14px;background:#f4faff}.takySummary b{font-size:12px;color:#3277a7}.takySummary span{font-size:12px;font-weight:850;color:#173f5f}.takyPrimary,.takyReset{border:0;border-radius:15px;padding:14px 16px;font-weight:900}.takyPrimary{background:#1769ff;color:#fff}.takyReset{background:#eef6fb;color:#315d79}@media(max-width:420px){.takyChoiceGrid{gap:7px}.takyChoiceCard{padding:6px;border-radius:16px}.takyChoiceCard strong{font-size:11px}.takyArt{border-radius:12px}}`;document.head.appendChild(style);
