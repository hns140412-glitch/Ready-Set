(() => {
  ...
  function explorerTerms(){
    ...
    const map=new Map([['타임어택 작전 설정','오늘의 탐험 준비'],['오늘 목표 정하기','오늘 탐험 정하기'],['작전 기록실','탐험 기록'],['작전 일지','탐험 일지'],['오늘의 작전 미리보기','오늘의 탐험 미리보기'],['작전 공유하기','가족에게 응원 요청'],['타임어택 START','탐험 시작'],['오늘의 작전 보고서','오늘의 탐험 기록'],['오늘의 작전','오늘의 탐험']]);
  ...
  function hideLegacyChildSetup(){
    ...
    const homeShare=document.getElementById('preShareBtn');if(homeShare)homeShare.style.display='none';
  }
  ...
  function renderParent(){
    const main=document.querySelector('#missionView main');if(!main)return;const h=document.querySelector('#missionView .pageHeader h1');if(h)h.textContent='숙제 입력 · 확인';
  ...
  function patchHome(){
    ...
    if(ROLE==='PARENT')document.querySelectorAll('[data-nav="mission"]').forEach(b=>{const strong=b.querySelector('b');if(strong)strong.textContent='숙제 입력 · 확인';const small=b.querySelector('small');if(small)small.textContent='실제 숙제 정보만 입력'});
    else{const b=document.querySelector('.commandCard [data-nav="mission"] b');if(b)b.textContent='오늘 탐험 정하기';const s=document.querySelector('.commandCard [data-nav="mission"] small');if(s)s.textContent='오늘 할 일을 고르고 목표 시간을 정해요'}
  }

  function boot(){
    document.documentElement.dataset.readyStageF=VERSION;document.documentElement.dataset.readyRole=ROLE;styles();patchHome();explorerTerms();hideLegacyChildSetup();if(ROLE==='PARENT')renderParent();else renderChild();window.ReadyStageF=Object.freeze({version:VERSION,role:ROLE,render:()=>ROLE==='PARENT'?renderParent():renderChild()});
  }
