(() => {
  'use strict';
  const VERSION='2026.09.10-stage-base-ui-v7.10';
  const SOURCES=['./ready-stage-c-base.js','./ready-stage-d.js','./ready-stage-e.js','./ready-stage-f.js','./ready-stage-g1-fix.js','./ready-stage-g14-planner-authority.js','./ready-base-native-v2.js','./ready-planner-selection-bridge-v1.js','./ready-focus-tools-v1.js','./ready-schedule-base-v1.js','./ready-world-base-v1.js','./ready-onboarding-identity-v1.js','./ready-character-candidate-v1.js','./ready-world-shell-v1.js','./ready-base-selftest-v1.js'];
  ...
  const recoverMobileView=reason=>{
    ...
    const now=Date.now(),key='ready_mobile_recovery_reload_at',last=Number(sessionStorage.getItem(key)||0);
    if(now-last>5000){
      sessionStorage.setItem(key,String(now));
      document.documentElement.dataset.readyMobileRecovery='reload';
      location.reload();
    }
  };
  (async()=>{
    ...
    if(!r?.nativeRuntime||r?.legacyAppJsRequired!==false||!r?.plannerIdBinding||!r?.plannerSelectedIsAuthority||!r?.completionWritesPlanner||!r?.timeOptional||!r?.completedTaskNotActive||!r?.resultStatusSingle)throw new Error('BASE_UI_V710_NATIVE_RUNTIME_MISSING');
    if(!b?.stablePlannerId||!b?.coreBridge)throw new Error('BASE_UI_V710_SELECTION_BRIDGE_MISSING');
    if(!f?.recordingDefaultHidden||!f?.onDemandTool)throw new Error('BASE_UI_V710_FOCUS_TOOLS_MISSING');
    if(!s?.weekly||!s?.daily||!s?.plannerLinked||!s?.noFreeTimeInference)throw new Error('BASE_UI_V710_SCHEDULE_MISSING');
    if(!wb?.nativeWorldBase||!wb?.noCardShell||!wb?.sceneTransition||wb?.legacyBaseUnderlay!==false)throw new Error('BASE_UI_V710_WORLD_BASE_INVALID');
    if(!id?.firstRunGate||!id?.nativeFirstPaintRelease||!id?.identityMasterContract||!id?.visualIdRequired||!id?.photoIsNotVisualId||!id?.legalNameSeparate||!id?.nicknamePrimary||!id?.shareNamePolicy||!id?.birthDateRequired||!id?.schoolStageRequired||!id?.characterTimeline||!id?.annualRefreshOptional||!id?.seasonalLayerContract||!id?.worldPlaceOptionalAfterArrival||!id?.profileForm||!id?.photoCapture||!id?.photoResize||!id?.characterApprovalGate||!id?.readyValidationBeforeWrite||!id?.readyMissingFactsReported)throw new Error('BASE_UI_V710_IDENTITY_CONTRACT_INVALID');
    if(!cc?.threeCandidateContract||!cc?.noFakeGeneration||!cc?.sourcePhotoIsIdentitySource||!cc?.explicitApprovalRequired||!cc?.versionedIdentity||!cc?.similarFlow||!cc?.resetFlow||!cc?.backendRequired||!cc?.kidFacingCopy||!cc?.fullBodyCandidateLayout||!cc?.visualIdApprovalGate||!cc?.costConfirmation||!cc?.serverSideAdapter||!cc?.personalityDirections||!cc?.sameIdentitySameQuality||!cc?.highQualityCandidates||!cc?.candidateChoiceIsDirection||!cc?.selectionIsNotConfirmation||!cc?.refinementGate||!cc?.finalApprovalAfterRefinement||!cc?.originalPhotoRemainsAuthority||!cc?.masterEndpointConnected||!cc?.separateMasterCostApproval||!cc?.masterGenerationIsNotApproval)throw new Error('BASE_UI_V710_CHARACTER_REFINEMENT_INVALID');
    if(!w?.enabled||!w?.plannerDriven||w?.legacyCategoryGrid!==false||!w?.nativeFirstPaint||!w?.journeyScene||!w?.arrivalScene||!w?.visualIdSlots)throw new Error('BASE_UI_V710_WORLD_SCENE_INVALID');
    if(e?.minuteCapacityAuthority!==false||e?.nextTuesdayNormalSlot!==false)throw new Error('BASE_UI_V710_PLANNER_SEMANTICS_MISSING');
    if(!g?.wrappedModel||g?.talentSourceTuesdayActiveTasks!==0)throw new Error('BASE_UI_V710_REGRESSION_FIX_MISSING');
    if(window.ReadyStageG13)throw new Error('BASE_UI_V710_G13_RUNTIME_STILL_LOADED');
    if(!j?.scheduleAuthority||!j?.noFreeTimeInference||!j?.scienceConditionalCycle||!j?.englishNextClassFromTimetable)throw new Error('BASE_UI_V710_PLANNER_AUTHORITY_MISSING');
    if(!n?.nativeHome||!n?.legacyCategoryAbsent||!n?.plannerSelectionBridgesCore||!n?.internalMetadataHidden)throw new Error('BASE_UI_V710_NATIVE_HOME_MISSING');
    if(document.getElementById('legacyCompatibility'))throw new Error('BASE_UI_V710_LEGACY_COMPATIBILITY_STILL_PRESENT');
    if([...document.scripts].some(x=>/\/app\.js(?:$|\?)/.test(x.src)))throw new Error('BASE_UI_V710_LEGACY_APP_JS_STILL_LOADED');
    const selftest=window.ReadyBaseSelfTestV1.run();if(!selftest?.pass)throw new Error(`BASE_UI_V710_SELFTEST_FAIL:${selftest?.failures?.map(x=>x.name).join(',')||'UNKNOWN'}`);
